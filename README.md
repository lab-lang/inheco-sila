...
This crate covers Inheco's SiLA-controlled instruments — today the ODTC
(On-Deck Thermal Cycler). The SCILA incubator speaks the same transport
and belongs here when implemented; Inheco's ThermoShake and CPAC use an
unrelated TEC control-box protocol and would be a sibling crate. It is
an independent implementation, not affiliated with or endorsed by
Inheco.
